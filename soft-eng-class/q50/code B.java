...
protected byte[] generateGlue(Collection<Executable> members) {
    ...
    cw.visitField(PRIVATE | FINAL, "index", INT_DESCRIPTOR, null, null).visitEnd();
    ...
    mv.visitFieldInsn(PUTFIELD, proxyName, "index", INT_DESCRIPTOR);
    mv.visitInsn(RETURN);
    mv.visitMaxs(0, 0);
    mv.visitEnd();
    ...
    mv.visitVarInsn(ALOAD, 0);
    mv.visitFieldInsn(GETFIELD, proxyName, "index", INT_DESCRIPTOR);
